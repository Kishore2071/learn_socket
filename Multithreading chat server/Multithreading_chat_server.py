...
4. The server has to take the user's name before letting him chat. 
...
class ChatServerOutgoingThread(Thread):
    ...
    def killThread(self, should_inform=False):
        # Inform others that this user has disconneced.
        self.can_kill = True
...
class ChatServerIncomingThread(Thread):
    ...
    def run(self):
        ...
        while not self.conn._closed:
            data = self.conn.recv(1024)
            if not data:  # Means, the client has disconneced.
...
sock = s.socket(s.AF_INET,
                s.SOCK_STREAM)  # AF_INET means Domain name or IPv4 Address and SOCK_STREAM means TCP communication.
